    ...
    $(function(e) {
        $('body').on('click', '#btn_add', function(e) {
            ...
            var li  = '<li data-id=""><span class="handle ui-sortable-handle">' +
                      ...
                      '</span><input type="textbox" name="txt_category" class="form-control txt-n-s"/><span class="text"></span>' +
                      '<div class="tools">' +
                      '<i class="fa fa-edit edit-icon" data-target="false"></i>' +
        ...
        $('body').on('click', '.edit-icon', function(e) {
            var params = {
                ...
                li: $(this).closest('li'),
                text_box: '<input type="textbox" name="txt_category" class="form-control"/>'
            };
        ...
        $('body').on('click','#btn_save', function(e) {
            ...
            if (li.length == 0) {
                ...
                $.each(li, function(i, v) {
                    var text = $(this).find('input[name="txt_category"]');
                    if (text.val() == '') {
                        $(this).find('.text').addClass('text-red').append('* not be blank!');
                    } else {
                        var arr = {
                ...
                $.post('<?php echo $this->Url->build('/admin/categories/add_sub_category'); ?>', {'data': data, '_csrfToken': _csrfToken,'id': parent_id}, function(data) {
                    location.reload();
                }, 'json');
            }
        });
